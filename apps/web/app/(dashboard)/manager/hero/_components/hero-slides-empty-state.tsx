...
export function HeroSlidesEmptyState({
  hasQuery,
  canCreate,
  onCreate,
}: {
  ...
  return (
    ...
    >
      <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-2xl border border-brand/25 bg-brand/10">
        <Presentation className="h-7 w-7 text-brand" aria-hidden />
      </div>
      {hasQuery ? (
        <>
        ...
        <>
          ...
          <p className="mt-1 max-w-md text-sm leading-7 text-muted-foreground">
            با ایجاد اولین اسلاید، محتوای وب‌سایت عمومی خود را مدیریت کنید.
          </p>
          {canCreate && onCreate ? (
