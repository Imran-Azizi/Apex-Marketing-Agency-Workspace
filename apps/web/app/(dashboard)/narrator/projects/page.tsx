...
export default function NarratorProjectsPage() {
  ...
  const { data, isLoading, error, isFetching } = useQuery({
    ...
    queryFn: () => {
      ...
      return apiGet<ProjectsResponse>(`/narration/projects?${params.toString()}`);
    },
  });
  ...
  return (
    <div className="space-y-6 animate-fade-slide" dir="rtl">
          ...
          <div className="min-w-0 flex-1 space-y-1.5">
            ...
            <div className="relative">
              ...
              <Input
                ...
                placeholder="عنوان، متن نریشن یا وضعیت…"
      ...
      <div className="flex items-center justify-between gap-2 text-sm text-muted-foreground">
        <p>
          {data
            ? `${data.total.toLocaleString("fa-AF", { numberingSystem: "latn" })} نریشن`
            : "—"}
      ...
      {isLoading ? (
        ...
        <EmptyState title="بارگذاری نریشن‌ها ناموفق بود" />
      ) : items.length === 0 ? (
        <EmptyState
          title="نتیجه‌ای یافت نشد"
          description="فیلترها را تغییر دهید یا منتظر ارسال نریشن جدید توسط مدیر بمانید."
        />
      ) : view === "grid" ? (
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
          {items.map((task) => (
