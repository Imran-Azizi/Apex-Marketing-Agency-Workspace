...
export function CustomerFormDialog({
  ...
  const mutation = useMutation({
    mutationFn: (values: FormValues) => {
      ...
      if (isEdit && customer) {
        return apiPatch(`/crm/customers/${customer.id}`, payload);
      }
      return apiPost("/crm/customers", payload);
    },
    onSuccess: () => {
      toast.success(
        isEdit ? "اطلاعات سرنخ به‌روزرسانی شد" : "سرنخ با موفقیت ایجاد شد"
      );
      invalidate();
      onOpenChange(false);
    },
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle>{isEdit ? "ویرایش سرنخ" : "سرنخ جدید"}</DialogTitle>
          <DialogDescription>
            {isEdit
              ? "اطلاعات سرنخ را ویرایش کنید."
              : "سرنخ جدید را با شماره واتساپ ثبت کنید. اگر نام مشخص نیست، می‌توانید بعداً تکمیل کنید."}
          </DialogDescription>
        </DialogHeader>
        <form
        ...
        >
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              <Label htmlFor="personName">نام شخص</Label>
