...
class MorphEngine {
  ...
  constructor(
    ...
    this.gl.clearColor(0.05, 0.06, 0.09, 1);
  ...
  goToIndex(index: number) {
    if (this.animating || this.dragging || this.items.length < 1) return;
  ...
  destroy() {
    ...
    this.textures.forEach((tex) => {
      const textured = tex as Texture & { texture?: WebGLTexture };
      if (textured.texture) this.gl.deleteTexture(textured.texture);
    });
    const programmed = this.program as Program & { program?: WebGLProgram };
    if (programmed.program) this.gl.deleteProgram(programmed.program);
...
export type MorphSliderProps = {
  ...
  className?: string;
  paused?: boolean;
  /** Pause autoplay while the pointer is over the slider. Default true. */
  pauseOnHover?: boolean;
  /** Enable arrow-key navigation on the stage. Default true. */
...
const MorphSlider = forwardRef<MorphSliderHandle, MorphSliderProps>(
  function MorphSlider(
    ...
    const optsRef = useRef<MorphEngineOptions>({
      transition,
      duration,
      ease,
      intensity,
    ...
    const itemsKey = items.map((item) => item.image).join("|");
    ...
    const handleGoToIndex = useCallback(
      (next: number) => engineRef.current?.goToIndex(next),
      [],
    );
    ...
    useImperativeHandle(
      ...
      [handleGoToIndex, handleNext, handlePrev],
    );

    useEffect(() => {
      if (!autoplay || paused || (pauseOnHover && hovering)) return undefined;
      const id = setTimeout(
        () => engineRef.current?.next(),
        Math.max(autoplayDelay, 1) * 1000,
      );
      return () => clearTimeout(id);
    }, [autoplay, autoplayDelay, hovering, pauseOnHover, paused, index]);

    useEffect(() => {
      if (!enableDrag) return undefined;
    ...
    const onKeyDown = useCallback(
      ...
      [handleNext, handlePrev, keyboardNavigation],
    );
    ...
    const style: CSSProperties = {
      ...
      ["--ms-swap" as string]: `${(duration * 0.66).toFixed(3)}s`,
      ["--ms-dot" as string]: `${(duration * 0.45).toFixed(3)}s`,
      ...(props.style || {}),
    };
