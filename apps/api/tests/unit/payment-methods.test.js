...
test('customer payments allow the supported methods', () => {
  ...
  assert.equal(isCustomerPaymentMethod('BANK_TRANSFER'), true);
...
test('payment API requires a supported method on create', () => {
  ...
  for (const method of [undefined, 'OTHER', 'UNKNOWN']) {
    const payload = {
      opportunityId: 'opportunity-id',
      amount: 100,
      ...(method ? { method } : {}),
...
test('payment API permits editing to supported methods only', () => {
  ...
  assert.equal(updatePaymentSchema.safeParse({ method: 'BANK_TRANSFER' }).success, true);
  assert.equal(updatePaymentSchema.safeParse({ amount: 200 }).success, true);
  assert.equal(
    updatePaymentSchema.safeParse({ method: 'OTHER' }).success,
    false,
  );
});
