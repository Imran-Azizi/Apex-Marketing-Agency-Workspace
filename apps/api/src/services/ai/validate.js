 ...
 */
export function normalizeScenarioOutput(output, projectId, context = {}) {
  ...
  const lockedLanguage = String(context.language || o.language || 'fa').trim() || 'fa';
  const defaultCta =
    lockedLanguage === 'en' || lockedLanguage.startsWith('en')
      ? 'Contact us'
      : 'تماس بگیرید';
  ...
  const cta = requireNonEmptyString(picked.cta || o.cta || defaultCta, 'cta');
...
export function normalizeNarrationOutput(output, projectId, context = {}) {
  const o = asObject(output);
  if (!o) {
  ...
  // Language + tone are project selections — never invent / AI-pick them.
  const lockedLanguage = String(context.language || o.language || 'fa').trim() || 'fa';
  const lockedTone = String(context.tone || '').trim();
  ...
  return {
    ...
    language: lockedLanguage,
    tone: lockedTone,
    // Keep empty: tone is selected on the project, not generated by AI.
    toneExplanation: '',
    estimatedSeconds,
...
export function normalizeStoryboardOutput(output, projectId, context = {}) {
  ...
  return {
    projectId: o.projectId || projectId,
    language: String(context.language || o.language || 'fa').trim() || 'fa',
    tone: String(context.tone || '').trim(),
    visualStyleGuide:
...
export function normalizePipelineOutputs(outputs, projectId, context = {}) {
  if (!outputs || typeof outputs !== 'object') return outputs;
  return {
    scenario: normalizeScenarioOutput(outputs.scenario, projectId, context),
    narration: normalizeNarrationOutput(outputs.narration, projectId, context),
    storyboard: normalizeStoryboardOutput(outputs.storyboard, projectId, context),
  };
}
...
export function validateAgentOutput(agentType, output, projectId, context = {}) {
  if (agentType === 'SCENARIO') return normalizeScenarioOutput(output, projectId, context);
  if (agentType === 'NARRATION') return normalizeNarrationOutput(output, projectId, context);
  if (agentType === 'STORYBOARD') return normalizeStoryboardOutput(output, projectId, context);
  return output;
}
